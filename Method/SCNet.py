...
class Trainer(object):
    ...
    def predict(self, dataset, training=True):
        if training:
            self.model.train()
            loss_total = 0
            ...
            for data in dataset:
                ...
                input_var = batch_input.to(torch.float32)
                target_var = batch_target.to(torch.float32)

                batch_output = self.model(input_var).view(-1)
            ...
            with torch.no_grad():
                ...
                for data in dataset:
                    ...
                    input_var = batch_input.to(torch.float32)
                    target_var = batch_target.to(torch.float32)
                    predicted_scores = self.model(input_var)
                    T.append(target_var)
                    Y.append((predicted_scores >= 0.5).to(dtype=torch.int))
                    S.append(predicted_scores)

                T = torch.cat(T, dim=0).cpu()
                Y = torch.cat(Y, dim=0).cpu()
                S = torch.cat(S, dim=0).cpu()
